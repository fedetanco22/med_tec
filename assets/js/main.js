	...
	window.addEventListener("load", () => {
		...
		if (solucionesContainer) {
			let solucionesIsotope = new Isotope(solucionesContainer, {
				itemSelector: ".soluciones-item",
				filter: ".filter-active",
			});

			let solucionesFilters = select("#soluciones-flters li", true);

			on(
				"click",
				"#soluciones-flters li",
				function (e) {
					e.preventDefault();
					solucionesFilters.forEach(function (el) {
						el.classList.remove("filter-active");
					});
					this.classList.add("filter-active");

					solucionesIsotope.arrange({
						filter: this.getAttribute("data-filter"),
					});
					solucionesIsotope.on("arrangeComplete", function () {
						AOS.refresh();
					});
				},
				true
